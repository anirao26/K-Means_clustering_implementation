...
def cluster(X, y, k, n_starts=5):
    ...
    # Main function body
    print("Performing clustering for k = ", k)
...
def assign(X, Mu):
    ...
    z = np.repeat(-1, X.shape[0])
    for i in range(len(X)):
        dist_array = []
        for j in range(len(Mu)):
            dist = np.linalg.norm(X[i]-Mu[j])
            dist_array.append(dist)
        z[i] = dist_array.index(min(dist_array))
    return z
...
def update(X, z, k):
    ...
    Mu = np.ones((k, X.shape[1]))
    for i in range(k):
        array = []
        for j in range(len(z)):
            if z[j] == i:
                array.append(j)
        Mu[i, :] = np.mean(X[array, :], axis = 0)
...
def compute_distortion(X, Mu, z):
    ...
    for i in range(len(Mu)):
        ...
        for j in range(len(z)):
            if z[j] == i:
                sum+=np.square(np.linalg.norm(X[j]-Mu[i]))
        distortion+=sum
    return distortion
...
def label_clusters(y, k, z):
    ...
    labels = []
    labels_pd = pd.DataFrame({'z': z, 'y': y})
    
    for i in range(k):
        labels.append(labels_pd[labels_pd["z"]==i]["y"].value_counts().index[0])
